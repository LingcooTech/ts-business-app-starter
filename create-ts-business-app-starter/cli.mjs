...
const DEFAULT_REPOSITORY = 'LingcooTech/ts-business-app-starter';
...
function usage() {
  console.log(`
Usage:
  npx @lingcoo-tech/create-ts-business-app-starter@latest <directory> [options]
    ...
    for (const refType of refs) {
      ...
      try {
        ...
        const extracted = (await readdir(workdir)).find((entry) =>
          entry.startsWith('ts-business-app-starter-'),
        );
...
async function copyTemplate(source, target) {
  ...
  for (const entry of await readdir(source)) {
    ...
    await cp(join(source, entry), join(target, entry), {
      recursive: true,
      force: true,
      filter: (sourcePath) => !excludedEntries.has(basename(sourcePath)),
    });
  }
  await rm(join(target, '.git'), { recursive: true, force: true });
  await rm(join(target, 'create-ts-business-app-starter'), { recursive: true, force: true });
...
async function transformFiles(root, projectName) {
  ...
  for (const entry of entries) {
    ...
    const protectedGeneratorName = '__CREATE_TS_BUSINESS_APP_STARTER__';
    const transformed = content
      .replaceAll('create-ts-business-app-starter', protectedGeneratorName)
      .replaceAll('ts-business-app-starter', projectName)
      .replaceAll(protectedGeneratorName, 'create-ts-business-app-starter');
...
async function removeMaintainerOnlyFiles(root) {
  ...
  await rm(join(root, '.base-starter-version'), { force: true });
}
